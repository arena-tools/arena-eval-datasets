#!/usr/bin/env tsx
/**
 * Test Planning Dataset Upload Script
 *
 * Usage:
 *   npx tsx scripts/upload-test-planning.ts --file TP_generate_goals.csv
 *   npx tsx scripts/upload-test-planning.ts --all
 *   npx tsx scripts/upload-test-planning.ts --changed
 *   npx tsx scripts/upload-test-planning.ts --all --dry-run
 */
...
async function processFile(csvFilename: string, dryRun: boolean): Promise<void> {
  ...
  const output = convertTestPlanning(csvText, datasetName)
...
async function main() {
  ...
  if (fileArg) {
    ...
    try {
      csvFiles = await getChangedCsvFiles(DATASETS_DIR, 'datasets/test_planning/')
