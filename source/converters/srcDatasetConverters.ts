...
export function createFanoutDataset(rows: SrcRow[]): DatasetOutput {
  ...
  const fanoutByParent = new Map<string, string[]>()
  const ruleOrder: string[] = []
  ...
  for (const parent of ruleOrder) {
    const requirement = parentRules.get(parent)!
    const subruleElements = fanoutByParent.get(parent) || []
    outputRows.push({
      board_name: boardId,
      rule: requirement,
      expected_output: JSON.stringify({ verifiability: subruleElements.length > 0 ? 1 : 0, subrule_elements: subruleElements }),
    })
  }
...
export function createRuleDataset(rows: SrcRow[]): DatasetOutput {
  const boardId = rows[0]?.['Board Name'] || ''
  const parentRules = new Map<string, { requirement: string; status: string }>()
  const fanoutItems = new Map<string, { id: string; status: string }[]>()
  const ruleOrder: string[] = []
...
export function parseCsv(text: string): SrcRow[] {
  return parseCsvToObjects<SrcRow>(text)
}
